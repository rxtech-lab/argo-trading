...
type Trade struct {
	...
	// HoldTime is the holding time in seconds for a closing trade, calculated
	// against the prior unmatched entry trades. Under the fifo portfolio
	// calculation strategy the match consumes the oldest unmatched entries first;
	// under average_cost it consumes the most recently acquired entries first
	// (LIFO). For long positions it is computed on the sell trade; for short
	// positions, on the covering buy trade. It is the quantity-weighted average
